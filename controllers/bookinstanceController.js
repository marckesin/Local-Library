...
exports.bookinstance_delete_post = function (req, res) {
  ...
  BookInstance.findByIdAndDelete(req.body.bookinstanceid).exec((err, bookinstance) => {
    if (err) { next(err) }
    else {
      res.redirect('/catalog/bookinstances');
    }
...
exports.bookinstance_update_post = [
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      BookInstance.findByIdAndUpdate(req.params.id, bookinstance, {}).exec(function (err, bookinstance) {
        if (err) { next(err) }
        else {
          res.redirect(bookinstance.url);
        }
